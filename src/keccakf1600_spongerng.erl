...
init_from_buffer(In, Deterministic) when is_boolean(Deterministic) ->
	?WRAP_STATE(spongerng, keccakf1600:spongerng_init_from_buffer(In, Deterministic)).

init_from_file(Filename, Inlen, Deterministic) when is_boolean(Deterministic) ->
	?WRAP_STATE(spongerng, keccakf1600:spongerng_init_from_file(Filename, Inlen, Deterministic)).

init_from_dev_urandom() ->
	?WRAP_STATE(spongerng, keccakf1600:spongerng_init_from_dev_urandom()).

next({spongerng, State}, Outlen) ->
	?WRAP_STATE(spongerng, keccakf1600:spongerng_next(State, Outlen)).

stir({spongerng, State}, In) ->
	?WRAP_STATE(spongerng, keccakf1600:spongerng_stir(State, In)).
