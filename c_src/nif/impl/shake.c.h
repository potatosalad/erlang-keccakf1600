...
#define SHAKE_DEFINITION(bits)                                                                                                     \
    static int keccakf1600_nif_shake##bits##_2_work(ErlNifEnv *env, xnif_slice_t *slice, int *phasep, size_t *offsetp,             \
                                                    size_t reductions);                                                            \
    static ERL_NIF_TERM keccakf1600_nif_shake##bits##_2_done(ErlNifEnv *env, xnif_slice_t *slice);                                 \
                                                                                                                                   \
    static xnif_slice_func_t keccakf1600_nif_shake##bits##_2_func = {                                                              \
        keccakf1600_nif_shake##bits##_2_work,                                                                                      \
        keccakf1600_nif_shake##bits##_2_done,                                                                                      \
        NULL,                                                                                                                      \
        NULL,                                                                                                                      \
    };                                                                                                                             \
                                                                                                                                   \
    ...
    {                                                                                                                              \
        ...
        if (input.size <= XNIF_SLICE_MAX_PER_SLICE) {                                                                              \
            ...
            return out_term;                                                                                                       \
                                                                                                                                   ...
                                                                                                                                   \
        ctx = keccakf1600_nif_alloc_shake##bits##_ctx(env);                                                                        \
        if (ctx == NULL) {                                                                                                         \
            return enif_make_badarg(env);                                                                                          \
        }                                                                                                                          \
        (void)memcpy(ctx, &ctxbuf, sizeof(struct decaf_shake##bits##_ctx_s));                                                      \
                                                                                                                                   \
        xnif_slice_t *slice = xnif_slice_create(env, "shake" #bits, &keccakf1600_nif_shake##bits##_2_func, 0, input.size);         \
        if (slice == NULL) {                                                                                                       \
    ...
    {                                                                                                                              \
        ...
        ErlNifBinary input;                                                                                                        \
                                                                                                                                   \
        if (slice->argc != 3 || !keccakf1600_nif_get_shake##bits##_ctx(env, slice->argv[0], &ctx) ||                               \
    ...
    {                                                                                                                              \
        ...
        return out_term;                                                                                                           \
                                                                                                                                   ...
                                                                                                                                   \
    static int keccakf1600_nif_shake##bits##_update_2_work(ErlNifEnv *env, xnif_slice_t *slice, int *phasep, size_t *offsetp,      \
                                                           size_t reductions);                                                     \
    static ERL_NIF_TERM keccakf1600_nif_shake##bits##_update_2_done(ErlNifEnv *env, xnif_slice_t *slice);                          \
                                                                                                                                   \
    static xnif_slice_func_t keccakf1600_nif_shake##bits##_update_2_func = {                                                       \
        keccakf1600_nif_shake##bits##_update_2_work,                                                                               \
        keccakf1600_nif_shake##bits##_update_2_done,                                                                               \
        NULL,                                                                                                                      \
        NULL,                                                                                                                      \
    };                                                                                                                             \
                                                                                                                                   \
    ...
    {                                                                                                                              \
        ...
        ERL_NIF_TERM out_term;                                                                                                     \
                                                                                                                                   \
        if (argc != 2 || !keccakf1600_nif_get_shake##bits##_ctx(env, argv[0], &old_ctx) ||                                         \
            !enif_inspect_iolist_as_binary(env, argv[1], &input)) {                                                                \
            return enif_make_badarg(env);                                                                                          \
        }                                                                                                                          \
                                                                                                                                   \
        new_ctx = keccakf1600_nif_alloc_shake##bits##_ctx(env);                                                                    \
        if (new_ctx == NULL) {                                                                                                     \
            return enif_make_badarg(env);                                                                                          \
        }                                                                                                                          \
        ...
        if (input.size <= XNIF_SLICE_MAX_PER_SLICE) {                                                                              \
            ...
            return out_term;                                                                                                       \
                                                                                                                                   ...
                                                                                                                                   \
        xnif_slice_t *slice =                                                                                                      \
            xnif_slice_create(env, "shake" #bits "_update", &keccakf1600_nif_shake##bits##_update_2_func, 0, input.size);          \
        if (slice == NULL) {                                                                                                       \
            (void)enif_release_resource((void *)new_ctx);                                                                          \
            return enif_make_badarg(env);                                                                                          \
        }                                                                                                                          \
    ...
    {                                                                                                                              \
        ...
        ErlNifBinary input;                                                                                                        \
                                                                                                                                   \
        if (slice->argc != 2 || !keccakf1600_nif_get_shake##bits##_ctx(env, slice->argv[0], &ctx) ||                               \
            !enif_inspect_iolist_as_binary(env, slice->argv[1], &input)) {                                                         \
            return -1;                                                                                                             \
        }                                                                                                                          \
        (void)decaf_shake##bits##_update(ctx, (const uint8_t *)input.data + offset, reductions);                                   \
    ...
    {                                                                                                                              \
        ...
        return slice->argv[0];                                                                                                     \
                                                                                                                                   ...
                                                                                                                                   \
    static ERL_NIF_TERM keccakf1600_nif_shake##bits##_final_2_dirty(ErlNifEnv *env, int argc, const ERL_NIF_TERM argv[]);          \
                                                                                                                                   \
    ...
    {                                                                                                                              \
        ...
        out_term = enif_schedule_nif(env, "shake" #bits "_final", ERL_NIF_DIRTY_JOB_CPU_BOUND,                                     \
                                     keccakf1600_nif_shake##bits##_final_2_dirty, argc, argv);                                     \
                                                                                                                                   \
        return out_term;                                                                                                           \
    }                                                                                                                              \
                                                                                                                                   \
