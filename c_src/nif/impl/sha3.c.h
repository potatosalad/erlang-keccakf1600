...
#define SHA3_DEFINITION(bits, bytes)                                                                                               \
    static int keccakf1600_nif_sha3_##bits##_2_work(ErlNifEnv *env, xnif_slice_t *slice, int *phasep, size_t *offsetp,             \
                                                    size_t reductions);                                                            \
    static ERL_NIF_TERM keccakf1600_nif_sha3_##bits##_2_done(ErlNifEnv *env, xnif_slice_t *slice);                                 \
                                                                                                                                   \
    static xnif_slice_func_t keccakf1600_nif_sha3_##bits##_2_func = {                                                              \
        keccakf1600_nif_sha3_##bits##_2_work,                                                                                      \
        keccakf1600_nif_sha3_##bits##_2_done,                                                                                      \
        NULL,                                                                                                                      \
        NULL,                                                                                                                      \
    };                                                                                                                             \
                                                                                                                                   \
    ...
    {                                                                                                                              \
        ...
        if (input.size <= XNIF_SLICE_MAX_PER_SLICE) {                                                                              \
            ...
            return out_term;                                                                                                       \
                                                                                                                                   ...
                                                                                                                                   \
        ctx = keccakf1600_nif_alloc_sha3_##bits##_ctx(env);                                                                        \
        if (ctx == NULL) {                                                                                                         \
            return enif_make_badarg(env);                                                                                          \
        }                                                                                                                          \
        (void)memcpy(ctx, &ctxbuf, sizeof(struct decaf_sha3_##bits##_ctx_s));                                                      \
                                                                                                                                   \
        xnif_slice_t *slice = xnif_slice_create(env, "sha3_" #bits, &keccakf1600_nif_sha3_##bits##_2_func, 0, input.size);         \
        if (slice == NULL) {                                                                                                       \
    ...
    {                                                                                                                              \
        ...
        ErlNifBinary input;                                                                                                        \
                                                                                                                                   \
        if (slice->argc != 3 || !keccakf1600_nif_get_sha3_##bits##_ctx(env, slice->argv[0], &ctx) ||                               \
    ...
    {                                                                                                                              \
        ...
        return out_term;                                                                                                           \
                                                                                                                                   ...
                                                                                                                                   \
    static int keccakf1600_nif_sha3_##bits##_update_2_work(ErlNifEnv *env, xnif_slice_t *slice, int *phasep, size_t *offsetp,      \
                                                           size_t reductions);                                                     \
    static ERL_NIF_TERM keccakf1600_nif_sha3_##bits##_update_2_done(ErlNifEnv *env, xnif_slice_t *slice);                          \
                                                                                                                                   \
    static xnif_slice_func_t keccakf1600_nif_sha3_##bits##_update_2_func = {                                                       \
        keccakf1600_nif_sha3_##bits##_update_2_work,                                                                               \
        keccakf1600_nif_sha3_##bits##_update_2_done,                                                                               \
        NULL,                                                                                                                      \
        NULL,                                                                                                                      \
    };                                                                                                                             \
                                                                                                                                   \
    ...
    {                                                                                                                              \
        ...
        ERL_NIF_TERM out_term;                                                                                                     \
                                                                                                                                   \
        if (argc != 2 || !keccakf1600_nif_get_sha3_##bits##_ctx(env, argv[0], &old_ctx) ||                                         \
            !enif_inspect_iolist_as_binary(env, argv[1], &input)) {                                                                \
            return enif_make_badarg(env);                                                                                          \
        }                                                                                                                          \
                                                                                                                                   \
        new_ctx = keccakf1600_nif_alloc_sha3_##bits##_ctx(env);                                                                    \
        if (new_ctx == NULL) {                                                                                                     \
            return enif_make_badarg(env);                                                                                          \
        }                                                                                                                          \
        ...
        if (input.size <= XNIF_SLICE_MAX_PER_SLICE) {                                                                              \
            ...
            return out_term;                                                                                                       \
                                                                                                                                   ...
                                                                                                                                   \
        xnif_slice_t *slice =                                                                                                      \
            xnif_slice_create(env, "sha3_" #bits "_update", &keccakf1600_nif_sha3_##bits##_update_2_func, 0, input.size);          \
        if (slice == NULL) {                                                                                                       \
            (void)enif_release_resource((void *)new_ctx);                                                                          \
            return enif_make_badarg(env);                                                                                          \
        }                                                                                                                          \
    ...
    {                                                                                                                              \
        ...
        ErlNifBinary input;                                                                                                        \
                                                                                                                                   \
        if (slice->argc != 2 || !keccakf1600_nif_get_sha3_##bits##_ctx(env, slice->argv[0], &ctx) ||                               \
            !enif_inspect_iolist_as_binary(env, slice->argv[1], &input)) {                                                         \
            return -1;                                                                                                             \
        }                                                                                                                          \
        (void)decaf_sha3_##bits##_update(ctx, (const uint8_t *)input.data + offset, reductions);                                   \
    ...
    {                                                                                                                              \
        ...
        ERL_NIF_TERM out_term;                                                                                                     \
                                                                                                                                   \
        if (argc != 2 || !keccakf1600_nif_get_sha3_##bits##_ctx(env, argv[0], &old_ctx) ||                                         \
            !enif_get_ulong(env, argv[1], &outlen) || outlen > bytes) {                                                            \
            return enif_make_badarg(env);                                                                                          \
        }                                                                                                                          \
                                                                                                                                   \
        (void)memcpy(new_ctx, old_ctx, sizeof(struct decaf_sha3_##bits##_ctx_s));                                                  \
